...
You'll need to setup proxy forwarding with a web server to avoid CORS issues when running the React development server on port `3000`. If you are using NGINX, add the following location blocks to your main server block in the NGINX config:      
```
location / {
...
At this point all you need to do is run `npm install` and `npm start` from both the `client` and `server` folders. Make sure to use the URL `localhost:80` and NOT `localhost:3000` in the browser when visiting the site, otherwise you'll run into the CORS issues previously mentioned.
